...
We are given a small sandbox program that monitors a child process and checks that it doesn't use `execve`. We'll need to read the password file using `open` and `read` instead.
...
00001000200030004000500060007000800090000100110021003100410051006100710081009100020012002200320042005200620072008200920003001300230033004300530063007300830093000400140024003400440054006400740084009400

Program received signal SIGSEGV, Segmentation fault.
[Switching to process 1829]
0x30303339 in ?? ()
```

set follow-fork-mode child

segfault at offset 39*4 = 156 